...
with c1:
    ...
    <div class="metric-card">
        ...
        <p class="metric-value neutral">{profile['shape']['rows']:,}</p>
...
with c2:
    ...
    <div class="metric-card">
        ...
        <p class="metric-value neutral">{profile['shape']['columns']}</p>
    </div>""", unsafe_allow_html=True)

with c3:
    warn_class = "warn" if profile['total_missing'] > 0 else "good"
    st.markdown(f"""
    <div class="metric-card">
        <p class="metric-label">Missing Values</p>
        <p class="metric-value {warn_class}">{profile['total_missing']:,}</p>
    </div>""", unsafe_allow_html=True)

with c4:
    dup_class = "warn" if profile['duplicate_rows'] > 0 else "good"
    st.markdown(f"""
    <div class="metric-card">
        <p class="metric-label">Duplicate Rows</p>
        <p class="metric-value {dup_class}">{profile['duplicate_rows']}</p>
    </div>""", unsafe_allow_html=True)
...
if missing_report:
    ...
    for col, action in missing_report.items():
        if "dropped" in action:
            tag = '<span class="tag tag-dropped">dropped</span>'
        elif "median" in action or "mode" in action:
            tag = '<span class="tag tag-filled">imputed</span>'
        elif "forward filled" in action:
            tag = '<span class="tag tag-forward">forward filled</span>'
        else:
            tag = '<span class="tag tag-flagged">flagged</span>'

        rows_html += f"""
        <div class="clean-row">
            <span class="clean-col-name">{col}</span>
            <span class="clean-action">{tag}{action}</span>
        </div>"""
...
if auto_mode:
    # initialize results so it's always defined
    results = {}
        ...
        with st.spinner("Agent is analyzing your dataset..."):
            try:
                ...
                with redirect_stdout(f):
                    results = run_agent(df_clean, profile)
...
if "results" in st.session_state:
    ...
    for i, tool_name in enumerate([k for k in results.keys() if not k.startswith("_")], 1):
        steps_html += f"""
        <div class="step-row">
            <span class="step-done">{i}</span>
            <span class="step-name">{tool_name.replace("_", " ")}</span>
        </div>"""
    ...
    if "summary_stats" in results and isinstance(results["summary_stats"], dict):
        ...
        for col, stats in results["summary_stats"].items():
            if isinstance(stats, dict):
                stats_data.append({"column": col, **stats})
        if stats_data:
            st.dataframe(
        ...
        for col, info in results["detect_outliers"].items():
            if isinstance(info, dict):
                outlier_data.append({
                    ...
                    "Z-score outliers": info["zscore_outliers"],
                    "status": "flagged" if info["likely_has_outliers"] else "clean",
                })
        if outlier_data:
            st.dataframe(
                pd.DataFrame(outlier_data).set_index("column"),
                use_container_width=True
            )
        st.markdown("---")
    ...
    if os.path.exists(output_dir):
        ...
        if plot_files:
            ...
            for i, plot_file in enumerate(sorted(plot_files)):
                with cols[i % 2]:
                    caption = plot_file.replace("_", " ").replace(".png", "")
                    st.image(
                        os.path.join(output_dir, plot_file),
                        caption=caption,
                        use_container_width=True,
                    )
            st.markdown("---")
